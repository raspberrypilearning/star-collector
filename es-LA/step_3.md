...
**Depuración:** Si recibe un error que dice `The variable collectSound of ControladorEstrella has not been assigned.` (La variable collectSound de ControladorEstrella no se ha asignado.) esto significa que no ha configurado el clip de audio en el Inspector:
* Asegúrese de que la variable `collectSound` sea `pública`

* Busque el componente `StarCollector` en el Inspector de su Estrella, haga clic en el círculo y seleccione el clip de sonido `Collect` de la ventana emergente.
--- /task ---
