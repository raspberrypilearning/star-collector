...
<div>
![La vue Game affichant une variable "stars" sur le canevas.](images/counting-stars.gif){:width="300px"}
</div>
</div>
...
Sélectionne le **Player** et dans Inspector clique sur **Add Component** et crée un nouveau script appelé `StarPlayer`. Ouvre ton nouveau script dans l'éditeur de code et crée une nouvelle variable appelée `stars` :
...
Le script `StarController` doit mettre à jour la variable `stars` sur le joueur à chaque fois qu'une étoile est collectée.

--- task ---

Ouvre ton script `StarController` et ajoute du code pour augmenter le nombre d'étoiles du joueur de 1 à chaque fois qu'une étoile est collectée.

Le paramètre `other` de la méthode `OnTriggerEnter` est défini sur le GameObject qui est entré en collision avec l'étoile. Tu peux l'utiliser pour accéder à la variable `stars` de `StarPlayer` :
...
**Test :** Exécute ta scène et collecte les trois étoiles. Regarde la variable publique `stars` dans la fenêtre Inspector du joueur pour vérifier que le nombre augmente de 1 à chaque fois que tu collectes une étoile :

![Inspector montrant la variable stars définie sur 3 en mode Play.](images/stars-inspector.png)


**Debogage :** Si tu ne vois pas la variable `stars` dans Inspector, assure-toi d'avoir enregistré ton script `StarPlayer.cs`.
...
Dans la fenêtre Inspector, pour le GameObject TextMeshPro, accède au composant **Text Input**. Modifie `New Text` en `Stars: 0` :

![La grande fenêtre de saisie de texte avec "Stars: 0" tapé.](images/stars-start-text.png)
...
Ouvre ton script `StarPlayer` et ajoute `using TMPro` en haut pour que ton script puisse utiliser `TMP_Text` :
...
Ajoute du code pour créer un objet TMP_Text appelé `StarText` :
...
Dans la fenêtre Inspector du Player pour le script `StarPlayer`, clique sur le cercle à côté de la propriété Texte Etoile et choisis **Stars Text** pour lier ton objet texte.
